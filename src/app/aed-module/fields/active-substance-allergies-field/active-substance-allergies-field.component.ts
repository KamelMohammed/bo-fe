...
@Component({
  selector: 'app-active-substance-allergies-field',
  templateUrl: './active-substance-allergies-field.component.html',
  styleUrls: ['./active-substance-allergies-field.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ActiveSubstanceAllergiesFieldComponent implements OnInit, OnChanges {
  @Input() field?: Field;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.field && this.field) {
      ...
      if (this.field.activesubstanceallergies) {
        if (this.field.activesubstanceallergies.readonly) {
          this.formControl.disable();
        } else {
          this.formControl.enable();
        }
        if (this.field.activesubstanceallergies.values) {
          this.selectedOptions = this.field.activesubstanceallergies.values.filter(v => v);
  ...
  service$ = (value: string) => {
    if (value.length >= 3) {
      ...
      return this.field && this.field.activesubstanceallergies && this.field.activesubstanceallergies.service
        ? this.http.get<ServiceResponse[]>(`${environment.services.api.serviceBasePath}${this.field.activesubstanceallergies.service}/${value}`)
          .pipe(catchError(() => of([])))
        : of([]);
    } else {
      return of([]);
