...
export interface CommercialDrugAllergiesFieldOption {
  label: string;
  value: string;
}

@Component({
  selector: 'app-commercial-drug-allergies-field',
  templateUrl: './commercial-drug-allergies-field.component.html',
  styleUrls: ['./commercial-drug-allergies-field.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CommercialDrugAllergiesFieldComponent implements OnInit, OnChanges {
  @Input() field?: Field;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.field && this.field) {
      ...
      if (this.field.commercialdrugallergies) {
        if (this.field.commercialdrugallergies.readonly) {
          this.formControl.disable();
        } else {
          this.formControl.enable();
        }
        if (this.field.commercialdrugallergies.values) {
          this.selectedOptions = this.field.commercialdrugallergies.values.filter(v => v);
  ...
  service$ = (value: string) => {
    if (value.length >= 3) {
      ...
      return this.field && this.field.commercialdrugallergies && this.field.commercialdrugallergies.service
        ? this.http.get<ServiceResponse[]>(`${environment.services.api.serviceBasePath}${this.field.commercialdrugallergies.service}/${value}`)
          .pipe(catchError(() => of([])))
        : of([]);
    } else {
      return of([]);
